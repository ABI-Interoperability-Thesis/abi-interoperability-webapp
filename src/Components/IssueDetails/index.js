...
const IssueDetails = (props) => {
    ...
    const SendMessage = async (values) => {
        ...
        const data = {
            ...values,
            issue_id,
            sent_by_id: currentAcc.account_id,
            sent_by_name: currentAcc.username
        }
        ...
        <div style={{ height: '100%' }}>
            {
                ...
                <div style={{ height: '100%' }}>
                    ...
                    <div style={{ height: 'calc(100% - 10.5rem)', display: 'flex', flexDirection: 'column', gap: '1.5rem', overflowY: 'scroll' }}>
                        {
                            issueMsgs.map((msg) => (
                                <div style={{ width: '100%', display: 'flex', justifyContent: msg.sent_by_id === currentAcc.account_id ? 'flex-end' : 'flex-start' }}>
                                    <Card style={{ width: '50%' }} title={msg.sent_by_id === currentAcc.account_id ? 'You' : msg.sent_by_name}>
                                        <Paragraph>
                                            <pre>{msg.message}</pre>
                                        </Paragraph>
                                    </Card>
                    ...
                    <Form onFinish={SendMessage} style={{ width: '100%' }}>
                        <div style={{ display: 'flex', gap: '1rem', marginTop: '1rem', justifyContent: 'space-between' }}>
                            ...
                            <Form.Item name='message' style={{ width: '100%' }} rules={[
                                {
                                    required: true,
                                    message: 'Do not leave the message blank',
                                },
                            ]}>
